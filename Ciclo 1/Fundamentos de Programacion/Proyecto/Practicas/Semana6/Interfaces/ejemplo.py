...
def realizar_compra(cliente):
    ...
    if cliente.edad >= 10:
        ...
        if jugos_por_comprar > total_jugos_disponibles:
            messagebox.showinfo(
                "Atención", f"Hay {total_jugos_disponibles} y pidió {jugos_por_comprar}. Se le venderán {total_jugos_disponibles}.")
        ...
        label_jugos_disp["text"] = "Total de jugos disponibles: " + \
            str(total_jugos_disponibles)
        label_total_ventas["text"] = "Total ganado en ventas: " + \
            str(total_ventas)
