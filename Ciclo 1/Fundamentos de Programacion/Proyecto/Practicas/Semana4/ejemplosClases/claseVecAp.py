...
class altaPrecision(vector):
    ...
    def restaYacarreo(self, a, b, i, j, k):
        global acarreo
        res = 0
        if a >= b:
            res = a - b
    ...
    def __add__(self, b):
        ...
        while i > self.V[0]:
            ...
            k = k - 1
        while j > b.V[0]:
            r = self.sumaYacarreo(b.retornaDato(j), 0)
            resultado.asignaDato(r, k)
            j = j - 1
            k = k - 1
        if acarreo > 0:
    ...
    def __sub__(self, b):
        ...
        acarreo = 0
        while i > self.V[0] and j > b.V[0]:
            r = self.restaYacarreo(self.retornaDato(
                i), b.retornaDato(j), i, j, k)
            resultado.asignaDato(r, k)
            i = i - 1
            j = j - 1
            k = k - 1
        print("-----------")
        while i > self.V[0]:
            r = self.restaYacarreo(self.retornaDato(i), 0, i, j, k)
            resultado.asignaDato(r, k)
            i = i - 1
            k = k - 1
        while j > b.V[0]:
            r = self.restaYacarreo(b.retornaDato(j), 0, i, j, k)
            resultado.asignaDato(r, k)
            j = j - 1
            k = k - 1
